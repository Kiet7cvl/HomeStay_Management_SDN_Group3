...
import { buildImage, DEFAULT_USER, getUser, onErrorImg, readFile, readFileImg, URL_IMG } from "../../common/helper";
		...
		<div className="row">
			{
				images?.length > 0 && images.map((item, index) => {
					return (
						<div className='plus-image  col-md-3' key={index} onClick={e => {
							if (!item.imgBase64) {
						...
						}}>
							{
								item.imgBase64 ?
									<div>
										<img style={{width:"100%"}} src={item.file ? item.imgBase64 : URL_IMG + item.imgBase64} onError={onErrorImg}
