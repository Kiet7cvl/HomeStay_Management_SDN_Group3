	...
	return (
		<React.Fragment>
			<section className={`ftco-section ${props.notShowTitle ? '' : ' bg-light'}`}>
				<Container>
					{
						!props.notShowTitle && <Row className="row justify-content-center  py-3">
											...
											<Col lg={props.size || 4} md={props.size || 4} key={key}>
												<div className="room">
													...
													<div className="text p-3 text-center br-bottom-left-6 br-bottom-right-6">
														<h3 className="mb-3">
															<Link class="text-decoration-none" to={'/room/' + item._id}>{item.name}</Link>
															<StarIcons vote_number={avgStar} />
														</h3>
														<p>
															<span className="price mr-2">{customNumber(item.price, '.', ' vnđ')}</span>
